...
from .fish_enum import Waters, Time
from .. import ASSETS
...
async def chitung_fish_tool_handler(
    ...
    if function.result.display == "endfish":
        ...
        await app.send_group_message(event.sender.group, reply_msg)

    elif function.result.display == "collection":
    ...
    elif function.result.display == "fishhelp":
        image_path = Path(assets_dir / "fishinfo.png")
        await app.send_group_message(
            event.sender.group, MessageChain([Image(path=image_path)])
        )

    elif function.result.display == "handbook":
...
async def chitung_fish_handler(
    ...
    water = pool.result.display.strip().upper()
    if event.sender.id in fishing_process_flag:
        await app.send_group_message(event.sender.group, MessageChain("上次抛竿还在进行中。"))
    else:
        w = get_water(water)
        reply_msg = MessageChain(At(event.sender.id))
        if w != Waters.General:  # 非常规水域扣费
            if vault.has_enough_money(
                ...
                await app.send_group_message(
                    event.sender.group,
                    MessageChain(reply_msg + Plain(text="您的黄瓜比索数量不够，请检查。")),
                )
                return
        ...
        await app.send_group_message(event.sender.group, MessageChain(reply_msg))
        # 这就是钓鱼开始了，在这躺下，躺够了起来
        ...
        reply_msg = MessageChain(At(event.sender.id), Plain(text="您钓到了：\n\n"))
        total_value = 0
...
async def collection(app: Ariadne, group: Group, member: Member):
    ...
    reply_msg += Image(data_bytes=handbook_img)
    await app.send_group_message(group, reply_msg)
