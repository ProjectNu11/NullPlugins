...
async def wordle_help(app: Ariadne, group: Group):
    await app.send_group_message(
        ...
        MessageChain(
            ...
            "绿色块代表此单词中有此字母且位置正确\n"
            "猜出单词或用光次数则游戏结束\n"
            f"发起游戏：{config.func.prefix[0]}wordle -l=5 -d=SAT，其中-l/-length为单词长度，-d/-dic为指定词典，默认为5和CET4\n"
            f"中途放弃：{config.func.prefix[0]}wordle -g 或 {config.func.prefix[0]}wordle -giveup\n"
            f"查看数据统计：{config.func.prefix[0]}wordle -s 或 {config.func.prefix[0]}wordle -statistic\n"
            f"查看提示：{config.func.prefix[0]}wordle -hint\n"
