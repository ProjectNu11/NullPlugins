...
def msg_encoder(encoding=None):
    ...
    if encoding in (None, False):
        return Codec(lambda x: str(x), lambda x: str(x))
    else:
        return Codec(
...
class Kernel:
    ...
    def bootstrap(self, brain_file=None, learn_files=None, commands=None, chdir=None):
        """准备一个内核对象以供使用。
    ...
    def respond(self, input_: str, session_id: Optional[str] = None):
        ...
        try:
            ...
            for s in sentences:
                ...
                final_response += response + "  "  # ????????  the final response.?
    ...
    def _respond(self, input_, session_id):
        ...
        if len(input_stack) > self._maxRecursionDepth:
            if self._verboseMode:
                err = "警告: 超过最大递归深度！ (input='%s')" % self._cod.enc(input_)
                sys.stderr.write(err)
            return ""
    ...
    def _process_srai(self, elem, session_id):
        ...
        new_input = " ".join(split_chinese(new_input))
