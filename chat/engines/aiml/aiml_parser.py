...
class AimlHandler(ContentHandler):
    ...
    def _start_element(self, name, attr):
        if name == "aiml":
            ...
            it = ((str(k), str(v)) for k, v in attr.items())
            attr_dict = dict(it)
            self._validate_elem_start(name, attr_dict, self._version)
            # 将当前元素推入元素堆栈。
            self._elemStack.append([str(name), attr_dict])
            self._push_whitespace_behavior(attr)
    ...
    def _characters(self, ch):
        ...
        if self._state == self._STATE_InsidePattern:
            ...
            try:
                ...
                parent_attr = self._elemStack[-1][1]
                required, optional, can_be_parent = self._validInfo[parent]
                non_block_style_condition = parent == "condition" and not (
                ...
                elif parent == "random" or non_block_style_condition:
            ...
            try:
                text_elem_on_stack = self._elemStack[-1][-1][0] == "text"
            except IndexError:
    ...
    def _end_element(self, name):
        ...
        if name == "aiml":
            ...
            self._currentPattern = " ".join(split_chinese(self._currentPattern))
            key = (
    ...
    def _validate_elem_start(self, name, attr, version):  # noqa
        """测试在<template>元素内开始元素的有效性。如果标签无效，此函数将引发AimlParserError异常。否则，没有消息是好消息。"""

        # 检查元素的属性。 确保所有必需的属性都存在，并且其余的属性都是有效的选项。
        required, optional, can_be_parent = self._validInfo[name]
        for a in required:
