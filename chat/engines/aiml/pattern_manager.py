...
class PatternMgr:
    ...
    def save(self, filename):
        ...
        try:
            ...
            marshal.dump(self._templateCount, out_file)
            marshal.dump(self._botName, out_file)
            marshal.dump(self._root, out_file)
            out_file.close()
        except Exception as e:
            print(f"Error saving PatternMgr to file {filename} cause: \n{e}")
            raise
    ...
    def restore(self, filename):
        ...
        try:
            ...
            self._templateCount = marshal.load(in_file)
    ...
    def match(self, pattern, that, topic):
        ...
        # 将输入传递给递归调用
        pat_match, template = self._match(
            input_.split(), that_input.split(), topic_input.split(), self._root
        )
        return template

    def star(self, star_type, pattern, that, topic, index):
        """返回一个字符串，即由*匹配的模式部分。
        'star_type'参数指定要找到哪种星型。  合法值是：
              - “star”：匹配主要模式中的一个星号。
        ...
        if star_type == "star":
            ...
            pat_match = pat_match[
                pat_match.index(self._THAT) + 1 : pat_match.index(self._TOPIC)
            ]
            ...
            # unknown value
            raise ValueError("star_type must be in ['star', 'thatstar', 'topicstar']")
    ...
    def _match(self, words, that_words, topic_words, root):
            ...
            if len(that_words) > 0:
                ...
                try:
                    pattern, template = self._match(
                        that_words, [], topic_words, root[self._THAT]
                    )
        ...
        if self._UNDERSCORE in root:
            ...
            for j in range(len(suffix) + 1):
                suf = suffix[j:]
                pattern, template = self._match(
                    suf, that_words, topic_words, root[self._UNDERSCORE]
                )
        ...
        if first in root:
            pattern, template = self._match(
                suffix, that_words, topic_words, root[first]
            )
            if template is not None:
        ...
        if self._BOT_NAME in root and first == self._botName:
            pattern, template = self._match(
                suffix, that_words, topic_words, root[self._BOT_NAME]
            )
            if template is not None:
                new_pattern = [first] + pattern
                return new_pattern, template
        ...
        if self._STAR in root:
            ...
            for j in range(len(suffix) + 1):
                suf = suffix[j:]
                pattern, template = self._match(
                    suf, that_words, topic_words, root[self._STAR]
                )
                if template is not None:
