...
class Image(AbstractElement):
    ...
    async def async_resize(
        ...
        return await loop.run_in_executor(
            None, self.resize, size, resample, box, reducing_gap
        )
    ...
    async def async_draw_ellipse(self, bounds, width=1, antialias=4):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            None, self.draw_ellipse, self.pil_image, bounds, width, antialias
        )
    ...
    async def async_round(self):
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.round)
